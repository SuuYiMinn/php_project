...
<div class="w-full">
    ...
    ?>
        ...
        <!-- products card -->
        <a href="./detailpage.php?productid=<?= $jfu_products["id"] ?>" class="lg:w-60 w-40 lg:h-60  h-40 bg-white drop-shadow-md mt-2" id="product_<?= $jfu_products["id"] ?>">
            <!-- product image -->
            <div class="w-full lg:h-2/3 h-3/5 relative">
                <img src="../..<?= $jfu_products["p_photo_1"] ?>" alt="" class="w-full h-full">
            </div>
            <!-- product information -->
            <div class="flex justify-evenly lg:h-1/3 h-2/5 bg-yellow-50">
                <div>
                    <p class="text-center lg:text-[13px] font-semibold text-[11px]"><?= $jfu_products["p_title"] ?></p>
                    <p class=" lg:block text-xs text-center hidden "><?= $jfu_products["p_des"] ?></p>
                    <?php if (($jfu_products["p_discount"]) != null) { ?> <p class="text-orange-500 lg:text-xs text-[9px] mt-2 ml-2"><?= $jfu_products["p_discount"] ?><span>%off</span></p><?php } ?>
                </div>
                <div>
                    ...
                    <?php if (($jfu_products["p_discount"]) != null) { ?>
                        <p class="lg:text-xs text-center line-through text-[9px]">Ks <?= $jfu_products["p_sell_price"] ?></p> <?php } ?>
