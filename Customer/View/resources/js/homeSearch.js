  ...
  function product_search(search) {
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        $("#home_main_section").removeClass("overflow-x-hidden w-full");
        $("#home_main_section").addClass(" flex w-10/12 mx-auto flex-wrap justify-evenly h-full");
        ...
        for (const search_product of search_products) {
          ...
          $("#home_main_section").append(
            ` <a href="./detailpage.php?productid=${search_product["id"]}" class="lg:w-56 w-40 lg:h-72 h-60 bg-white drop-shadow-md mt-8 group products_to_add" id ="${search_product["id"]}">
...
function handleWishList() {
  ...
  for (let index = 0; index < wishlist.length; index++) {
    $(wishlist[index]).click(function (event) {
      ...
      var isLogin = $("#home_main_section").attr("islogin");
...
function handleAdd_toCart () {
  ...
  for (let index = 0; index < cart.length; index++) {
    $(cart[index]).click(function (event) {
      event.preventDefault();
      isLogin = $("#home_main_section").attr("islogin");
      let productIdString = $(all_products[index]).attr("id");
