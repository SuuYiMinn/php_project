...
if (!isset($_SESSION["user_account"])) {
    ...
    $sql1 = $pdo->prepare(
        ...
        "SELECT t_cart.id FROM t_cart LEFT JOIN t_cart_detail ON t_cart.id = t_cart_detail.cart_id WHERE t_cart.customer_id = :user"
    );
    ...
    if (count($userResult) != 0) {
        ...
        $pcheck_sql = $pdo->prepare("SELECT * FROM t_cart_detail WHERE cart_id = :cartId AND product_id = :productId");
        ...
        if (count($pcheck_result) != 0) {
            ...
            $_SESSION["cart_exist"] = "Product is already added to your account";

            header("Location: ../View/detailpage.php?productid=$product_id");
        } else {
        ...
        $sql4 = $pdo->prepare(
            ...
            "SELECT id FROM t_cart WHERE customer_id = :user"
        );
